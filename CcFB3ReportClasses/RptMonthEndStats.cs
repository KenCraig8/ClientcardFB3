...
{
    class RptMonthEndStats : IDisposable
    {
        TrxLogPeriodTotals clsMonthStats;
        bool _disposed = false;

        public RptMonthEndStats(TrxLogPeriodTotals dataTableIn)
        {
        ...
        public void createReport(string foodBankName, string reportMonth,
            string preparedBy, object saveAs, string templatePath, string imagePath)
        {
            ...
            try
            {
                ...
                string sBookMark = "FoodBankName";
                Object oBookMarkName = sBookMark;
                oWordDoc.Bookmarks[oBookMarkName].Range.Text = foodBankName;
                foreach (string file in files)
                {
                    FileInfo fi = new FileInfo(file);
                    sBookMark = fi.Name.Substring(0, fi.Name.Length - 4); 
                    oBookMarkName = sBookMark;
                    try
                    {
