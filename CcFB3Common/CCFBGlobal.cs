...
namespace ClientcardFB3
{
    public static class CCFBGlobal
    {
        ...
        public const string DefaultShortDateFmt = "MM/dd/yyyy";
        public const string DefaultShortTimeFmt = "hh:mm tt";
        public const string password = "19800612";
        public static string homeDrive = @"C:\";
        public static string pathCSFP = "~";
        public static string pathExe = "~";
        public static string pathExports = "~";
        ...
        public static string parmTbl_AddressID = "parm_AddressID";
        public static string parmTbl_BackPackSchool = "parm_BackPackSchool";
        public static string parmTbl_BackPackSize = "parm_BackPackSize";
        public static string parmTbl_Client = "parm_ClientType";
        public static string parmTbl_CSFPRoutes = "parm_CSFPRoutes";
        public static string parmTbl_CSFPSortOrder = "parm_CSFPSortOrder";
        public static string parmTbl_CSFPStatus = "parm_CSFPStatus";
        ...
        public static string parmTbl_Transportation = "parm_Transportation";
        ...
        //This should match the parm_ServiceMethod table
        {
            Pickup = 0,
            AlternatePickup = 1,
            HomeDeliveryActive = 2,
            HomeDeliveryOnhold = 3,
            Referred = 4,
            OneTimeService = 5
        }
        ...
        public static void verifyPath(string path)
        {
            ...
            for (int i = 1; i < pathParts.Length; i++)
            {
                ...
                {
                    appendErrorToErrorReport(pathToFind, "Create Path");
        ...
        public static void appendErrorToErrorReport(string funtionParams, string errorInfo)
        {
            ...
            string folderPath = CCFBGlobal.homeDrive + @"ClientcardFB3\Log";
        ...
        {
            string fileName = "ErrorLog.txt";
            string destFile = System.IO.Path.Combine(pathLog, fileName);
        ...
        private static string checkFileExists(string saveName, string strExtension, string saveLocation)
        {
            ...
            if (fil.Exists == true)
            {
                     ...
                     MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                {
                    ...
                    for (int i = 1; i < 999; i++)
                    {
                        fil = new FileInfo(saveLocation + saveName + strExtension);
                        if (fil.Exists == true)
                        {
        ...
        /// <param name="value">The value integer that needs formating</param>
        /// <returns>The string value with leading zeros</returns>
        public static string formatNumberWithSixLeadingZeros(object value)
        {
            try
            {
                Int64 convertedVal = Convert.ToInt64(value);
                if (convertedVal != 0)
                    return String.Format("{0:000000#}", convertedVal);
                else
                    return "0000000";
            }
        ...
        {
            pathExe = Application.StartupPath + @"\";
            if (System.Diagnostics.Debugger.IsAttached == true)
            {
                pathExe = homeDrive + @"ClientcardFB3\";
            }
            System.IO.DirectoryInfo di = new System.IO.DirectoryInfo(pathExe);
            ...
            foreach (System.IO.DirectoryInfo item in directories)
            {
                ...
                {
                    case "csfp":
                        pathCSFP = item.FullName + @"\";
                        break;
                    case "familycards":
                        pathFamilyCards = item.FullName + @"\";
                        break;
                    case "gridexports":
                        pathExports = item.FullName + @"\";
                        break;
                    case "log":
                        pathLog = item.FullName + @"\";
                        break;
                    case "reports":
                        pathReports = item.FullName + @"\";
                        break;
                    case "screenshots":
                        pathScreenshots = item.FullName + @"\";
                        break;
                    case "templates":
                        pathTemplates = item.FullName + @"\";
                        break;
                    default:
                        break;
                }
            }
            pathCSFP = setDefaultPath(pathCSFP, pathExe + "CSFP");
            pathExports = setDefaultPath(pathExports, pathExe + "GridExports");
            pathLog = setDefaultPath(pathLog, pathExe + "Log");
        ...
        {
            object dbValue = getSQLValue("if Exists(SELECT * FROM Household WHERE BarCode = " + barcodevalue + ")"
                              + " SELECT MAX(ID) FROM Household WHERE BarCode = " + barcodevalue + " ELSE SELECT 0");
            if (dbValue != null)
            {
                return (int)dbValue;
            }
        ...
        public static object getSQLValue(string sqlQuery)
        {
            try
            {
                SqlConnection sqlConn = new SqlConnection(connectionString);
            ...
            {
                appendErrorToErrorReport("CCFBGlobal.getSQLValue = " + sqlQuery,
        ...
        public static string lastDayOfMonth(string testDate)
        {
            string[] tmp = testDate.Split('/');
        ...
        {
            string tmp = "";
            if (CCFBPrefs.FilterPeriodFromAddress == true)
            {
                tmp = txt.Trim().Replace(".", "");
            }
            else
            {
                tmp = txt;
            ...
            tmp = tmp.Replace(" ne", " NE").Replace(" nw", " NW").Replace(" se", " SE").Replace(" sw", " SW");
            tmp = tmp.Replace(" street", " ST").Replace(" Street", " ST").Replace(" STREET", " ST");
            tmp = tmp.Replace(" st", " St").Replace(" Str", " St").Replace(" str", " St").Replace(" STR", " St");
        ...
        {
            return CCFBGlobal.currentUser_PermissionLevel == CCFBGlobal.permissions_Admin;
        }
        ...
        public static string FiscalPeriod(string testYYYYMM)
        {
            if (CCFBPrefs.FiscalYearStartMonth == 1)
            {
                return testYYYYMM.Substring(4);
            }
            int testmonth = Convert.ToInt32(testYYYYMM.Substring(4));
            if (testmonth >= CCFBPrefs.FiscalYearStartMonth)
            {
                return (Convert.ToInt32(testYYYYMM.Substring(0, 4)) + 1).ToString() + (testmonth - CCFBPrefs.FiscalYearStartMonth + 1).ToString("00");
            }
            return testYYYYMM.Substring(0, 4) + (13 - CCFBPrefs.FiscalYearStartMonth + testmonth).ToString("00");
        }

        public static int MonthDiff(DateTime dt1, DateTime dt2)
        {
            int MonthsElapsed = ((dt2.AddDays(1).Year - dt1.AddDays(1).Year) * 12) +
