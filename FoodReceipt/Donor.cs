...
namespace FoodReceipt
{
    public class Donors
    {
        ...
        public int DefaultDonationType
        {
            get
            {
                return drow.Field<System.Int16>("DefaultDonationType");
            }
            set
            {
        ...
        public bool open(int dnrID)
        {
            ...
            {
                closeConnection();
                CCFBGlobal.appendErrorToErrorReport("ID=" + dnrID.ToString(), ex.GetBaseException().ToString());
                iRowCount = 0;
                return isValid = false;
            }
        }
        public void openWhere(string sWhereClause = "")
        {
            ...
            {
                openConnection();
                command = new SqlCommand("SELECT * FROM " + tbName + sWhereClause, conn);
                dadAdpt.SelectCommand = command;
                dset.Clear();
                iRowCount = dadAdpt.Fill(dset, tbName);
                closeConnection();
