...
namespace ClientcardFB3
{
    public partial class frmFoodReceipts : Form
    {
                ...
                for (int i = 0; i < dgvMonthReceipts.SelectedCells.Count; i++)
                {
                    ...
                    && dgvMonthReceipts.SelectedCells[i].ColumnIndex < 8)
                    {
                        ...
                        if (dgvMonthReceipts[cellClmIndex, cellRowIndex].Tag != null)
                        {
                            ...
                            dset.Tables[0].Rows[dsetRowIndex]["ModifiedBy"] = CCFBGlobal.currentUser_Name;
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            CreateGroceryRescueReport clsCreateGRReport;
            string saveAs = "";
            if (pnlEditDaily.Visible == true)
            {
                clsCreateGRReport = new CreateGroceryRescueReport(dgvMonthReceipts);
            ...
            {
                clsCreateGRReport = new CreateGroceryRescueReport(dgvMonthTotals);
                saveAs = CCFBPrefs.ReportsSavePath + "GroceryRescue_" + cboReportMonth.SelectedItem
               + "_" + cboYear.SelectedItem + "_" + ((parmType)cboStore.SelectedItem).LongName + "_MontlyTotals.doc";
            }

            clsCreateGRReport.createReport(saveAs, @"C:\ClientcardFB3\Templates\GroceryRescue.doc",
                ((parmType)cboStore.SelectedItem).LongName);
        }
        ...
        private void cboReportMonth_SelectedIndexChanged(object sender, EventArgs e)
        {
