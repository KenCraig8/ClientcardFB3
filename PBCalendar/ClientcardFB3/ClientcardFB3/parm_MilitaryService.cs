...
{
    public class parm_MilitaryService
    {
        string connString;
        ...
        SqlCommand command;
        SqlConnection conn;
        static string tbName = "parm_MilitaryService";
        bool isValid = false;
        int iRowCount = 0;

        public parm_MilitaryService(string connStringIn)
        {
        ...
        {
            SqlCommand commDelete = new SqlCommand(" DELETE FROM parm_MilitaryService WHERE ID=" + key.ToString(), conn);
            command = new SqlCommand("SELECT * FROM parm_MilitaryService WHERE ID=" + key.ToString(), conn);
        ...
        public void update()
        {
            if (dset.HasChanges() == true)
            {
                try
                {
                    ...
                    dadAdpt.Update(dset, "parm_MilitaryService");
                    closeConnection();
                }
                catch (SqlException ex)
                {
