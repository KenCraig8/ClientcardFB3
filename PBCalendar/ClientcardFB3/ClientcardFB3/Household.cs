...
namespace ClientcardFB3
{
    public class Household
    {
        ...
        SqlCommand command;
        SqlConnection conn;
        static string tbName = "Household";
        bool isValid = false;
        int iRowCount = 0;
        DataRow drow;
        ...
        {
            get { return (DateTime)drow["DateIDVerified"]; }
        ...
        {
            SqlCommand sqlcmdObj = new SqlCommand("SELECT ID FROM " + tbName + " WHERE BarCode=" + barcode.ToString(), conn);
            openConnection();
            int id = (Int32)sqlcmdObj.ExecuteScalar();
            closeConnection();
            return id;
        }
        ...
        public void openWhere(string whereClause)
        {
            ...
            {
                openConnection();
                command = new SqlCommand("SELECT * FROM " + tbName + " Where " + whereClause + " Order By Name ASC", conn);
                dadAdpt.SelectCommand = command;
                dset.Clear();
                iRowCount = dadAdpt.Fill(dset, tbName);
                closeConnection();
        ...
        public int getHouseholdID(string name)
        {
            ...
            {
                openConnection();
                command = new SqlCommand("SELECT * FROM " + tbName + " Where Name='" + name + "'", conn);
                dadAdpt.SelectCommand = command;
                dset.Clear();
                iRowCount = dadAdpt.Fill(dset, tbName);
                closeConnection();
