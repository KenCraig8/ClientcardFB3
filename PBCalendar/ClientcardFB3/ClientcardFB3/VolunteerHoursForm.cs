...
namespace ClientcardFB3
{
    public partial class VolunteerHoursForm : Form
    {
        ...
        public VolunteerHoursForm()
        {
            ...
            dataGridViewVols.Columns["clmTimeIn"].Visible = CCFBPrefs.UseTimeInOutForVols;
            dataGridViewVols.Columns["clmTimeOut"].Visible = CCFBPrefs.UseTimeInOutForVols;
        ...
        private void lvDates_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                loadVolsGrid();
                dataGridViewVols.Visible = true;
            }
            catch(NullReferenceException ex) { }
        }

        private void dataGridViewVols_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridViewVols.Columns[e.ColumnIndex].Name == "clmTimeIn" ||
                dataGridViewVols.Columns[e.ColumnIndex].Name == "clmTimeOut")
            {
                dataGridViewVols["clmHrs", e.RowIndex].Value = 0;

                dataGridViewVols[e.ColumnIndex, e.RowIndex].Value = timeBeforeFormat;
                if (dataGridViewVols[1, e.RowIndex].Value != null
                    && dataGridViewVols[2, e.RowIndex].Value != null
                    && dataGridViewVols[1, e.RowIndex].Value.ToString().Trim() != ""
                    && dataGridViewVols[2, e.RowIndex].Value.ToString().Trim() != "")
                {
                    TimeSpan difference;
                    try
                    {
                        DateTime d = Convert.ToDateTime("1/1/2010 " + dataGridViewVols["clmTimeIn", e.RowIndex].Value.ToString().Trim());
                        DateTime d2 = Convert.ToDateTime("1/1/2010 " + dataGridViewVols["clmTimeOut", e.RowIndex].Value.ToString().Trim());
                        ...
                        dataGridViewVols["clmHrs", e.RowIndex].Value = difference.TotalHours.ToString("F");
                        setVolHrsInDatabase(e);
                        bSkipTotal = true;
                    }
                    catch (Exception ex) { }
                }
                else if (dataGridViewVols["clmHrs", e.RowIndex].Value.ToString() == "0"
                    && dataGridViewVols["clmVHID", e.RowIndex].Value != null)
                    setVolHrsInDatabase(e);
            }
            else
            {
        ...
        private void setVolHrsInDatabase(DataGridViewCellEventArgs e)
        {
            ...
            {
                int curVolID = 0;
                Single numHours = Convert.ToSingle(dataGridViewVols["clmHrs", e.RowIndex].Value);
                dataGridViewVols["clmHrs", e.RowIndex].Value = numHours.ToString("F");
                if (numHours == 0 && dataGridViewVols["clmVHID", e.RowIndex].Value != null)
                {
                    clsVolHrs.delete(int.Parse(dataGridViewVols["clmVHID", e.RowIndex].Value.ToString()));
                    dataGridViewVols["clmVHID", e.RowIndex].Value = null;
                }
                else if (numHours > 0 && dataGridViewVols["clmVHID", e.RowIndex].Value == null)
                {
                    curVolID = Convert.ToInt32(dataGridViewVols["clmVolID", e.RowIndex].Value);
                    DataRow drow = clsVolHrs.DSet.Tables[0].NewRow();
                    drow["VolID"] = dataGridViewVols["clmVolID", e.RowIndex].Value;
                    drow["TrxDate"] = DateTime.Parse(lvDates.FocusedItem.Text);
                    drow["NumVolunteers"] = 1;
                    drow["NumVolHours"] = numHours;
                    drow["VolTimeIn"] = dataGridViewVols["clmTimeIn", e.RowIndex].Value;
                    drow["VolTimeOut"] = dataGridViewVols["clmTimeOut", e.RowIndex].Value;
                    clsVolHrs.DSet.Tables[0].Rows.Add(drow);
                    if (clsVolHrs.insert())
                    {
                        clsVolHrs.findVolId(curVolID);
                        dataGridViewVols["clmVHID", e.RowIndex].Value = clsVolHrs.ID;
                    }
                }
                else
                {
                    ...
                    if (curVolID == clsVolHrs.VolId)
                    {
                        ...
                        if (dataGridViewVols["clmTimeIn", e.RowIndex].Value != null)
                            clsVolHrs.VolTimeIn = dataGridViewVols["clmTimeIn", e.RowIndex].Value.ToString();
                        else
                            clsVolHrs.VolTimeIn = "";
                        if (dataGridViewVols["clmTimeOut", e.RowIndex].Value != null)
                            clsVolHrs.VolTimeOut = dataGridViewVols["clmTimeOut", e.RowIndex].Value.ToString();
                        else
            ...
            catch (InvalidCastException ex) { dataGridViewVols[e.ColumnIndex, e.RowIndex].Value = 0; }
            catch (FormatException ex) { dataGridViewVols[e.ColumnIndex, e.RowIndex].Value = 0; }
        }

        private void dataGridViewVols_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridViewVols.Columns[e.ColumnIndex].Name == "clmHrs")
            {
                dataGridViewVols[e.ColumnIndex, e.RowIndex].Style.BackColor = Color.Yellow;
            }
        }

        private void VolunteerHoursForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            dataGridViewVols.EndEdit();
        }
        ...
        private void dataGridViewVols_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {

        }

        private void dataGridViewVols_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            if (dataGridViewVols.Columns[e.ColumnIndex].Name == "clmTimeIn" ||
                dataGridViewVols.Columns[e.ColumnIndex].Name == "clmTimeOut")
            {
                timeBeforeFormat = "";
                if (dataGridViewVols[e.ColumnIndex, e.RowIndex].
        ...
        {
            if (dataGridViewVols.Visible==true)
                loadVolsGrid();
        }
    }
}
