...
namespace ClientcardFB3
{
    public partial class HDPlannerForm : Form
    {
        ...
        HDRoutes clsHDRoute = new HDRoutes(CCFBGlobal.connectionString);
        HDRouteStatus clsHdRouteStatus = new HDRouteStatus(CCFBGlobal.connectionString);
        parmTypeCodes parmStatus = new parmTypeCodes(CCFBGlobal.parmTbl_RouteStatus,CCFBGlobal.connectionString);
        ...
        public HDPlannerForm(MainForm frmMainIn)
        {
            ...
            filllvwStatus();
            clsHDRoute.openWhere(" ORDER BY RouteTitle");
            if (clsHDRoute.RowCount > 0 )
            {
                cboHDRoute.DataSource = clsHDRoute.DTable;
...
//            EnableActionMenu();
        ...
        {
            clearRoutInfo();
            clsHDRoute.find(idx, true);
            btnSaveRoute.Visible = false;
            if (clsHDRoute.ID == idx)
            {
                tbDriver.Text = clsHDRoute.DriverName;
                tbPhone.Text = clsHDRoute.DriverPhone;
                tbEstTime.Text = clsHDRoute.EstHours.ToString("0.00");
                tbEstMiles.Text = clsHDRoute.EstMiles.ToString("0.0");
                tbRouteNotes.Text = clsHDRoute.Notes;
                tbDriverComments.Text = clsHDRoute.DriverNotes;
            }
        }
        ...
        private void tbRoute_Leave(object sender, EventArgs e)
        {
            ...
            {
                clsHDRoute.SetDataValue(tb.Tag.ToString(), tb.Text);
                setRouteDataChanged();
            }
        }
        ...
        {
            loadRouteList(dtpServiceDate.Value);
        }

        private void loadRouteList(DateTime svcdate)
        {
            clsHdRouteStatus.openWhere(svcdate.ToShortDateString());
            filllvwRouteStatus();
        }
        ...
        private void tsbPrepare_Click(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem item in lvwRouteStatus.Items)
            {
                ...
                {
                    cntr++;
                    createRouteStatusRow(Convert.ToInt32(item.Tag), svcDate);
                }
            }
            MessageBox.Show(cntr.ToString() + " route lists have been created for '" + svcDate.ToShortDateString() + "'", "Create Route Lists", MessageBoxButtons.OK); 
        }
        ...
        {
            if (clsHdRouteStatus.Add(RouteID, trxDate) < 0)
            {
        ...
        private void filllvwStatus()
        {
            ...
            for (int i = 0; i < parmStatus.TypeCodesArray.Count; i++)
            {
                string tmp = parmStatus.GetLongName(i);
                ListViewItem lvi = new ListViewItem(tmp);
                lvi.Tag = parmStatus.GetId(tmp);
                lvi.Checked = true;
