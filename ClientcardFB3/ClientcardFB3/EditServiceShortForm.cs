...
namespace ClientcardFB3
{
    public partial class EditServiceShortForm : Form
    {
        ...
        public void initForm(Client clsClientIn, bool ModeIn, bool isApptIn, int TrxIndexIn, string hhMemName)
        {
            ...
            for (int i = 0; i < clsClient.clsHHmem.RowCount; i++)
			{
                ...
                if (clsClient.clsHHmem.LastName + ", " + clsClient.clsHHmem.FirstName == hhMemName)
                    tmpPtr = i;
			}
            if (ptList.Count > 0)
            {
            ...
            if (TrxIndexIn < 0)
            {
                ...
                clsTrxItem = new TrxLogItem(clsTrxLog.DSet.Tables[0].NewRow(),clsClient.clsHH, hhMemName);
                if (isAppt == true)
                {
            ...
            pnlCSFP.Visible = clsClient.clsHHmem.HasCSFP && CCFBPrefs.EnableCSFP;
            fillForm();
        }
            ...
            for (int i = 0; i < clsClient.clsHHmem.RowCount; i++)
            {
                ...
                if (clsClient.clsHHmem.CSFP == true)
                {
                    if (clsClient.clsHHmem.CSFPExpiration >= monthStart)
                    {
                        ...
                        if (isNewTrx == true && lastService < monthStart)
                            lvi.Checked = true;
                        if (clsClient.clsHHmem.CSFPExpiration > Convert.ToDateTime(CCFBGlobal.OURNULLDATE))
                        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            //if (cboHHMem.SelectedValue == null)
        ...
        private void lvwFoodSvcItems_ItemChecked(object sender, ItemCheckedEventArgs e)
        {
            if (loadingControls == false && bProcessItemChecked == false)
            {
                ...
                {
                    si.IsSelected = e.Item.Checked;
                    if (si.Exclusive == true && e.Item.Checked == true)
