	...
	public class SqlGridDataSet : SqlBaseDataSet
	{
		...
		public void ClearFields (string a_value)
		{
			...
			{												//   the current tab control.
				ControlValue (m_map.m_uiControl[i], a_value);
			}
// &&&&& fix this later
//			m_map.m_uiControl[0].Focus();					// Put focus on 1st field if it is valid.
		...
		/// <param name="a_value">Set all fields on the page to this value.</param>
		//-----------------------------------------------------------------------------------------
		public string ControlValue (Control a_control)
		{
			...
			switch (a_control.GetType().Name.ToLower())
			{
				...
				case "combobox":	return (a_control.Text);
				case "textbox":		return (a_control.Text);
			}
			MessageBox.Show (
		...
		/// <param name="a_value">Set all fields on the page to this value.</param>
		//-----------------------------------------------------------------------------------------
		public bool ControlValue (Control a_control, string a_value)
		{
			...
			switch (a_control.GetType().Name.ToLower())
			{
				case "checkbox":
					...
					break;
				case "combobox":  a_control.Text = a_value;		break;
				case "textbox":	  a_control.Text = a_value;		break;
				default:
					status = false;
					MessageBox.Show (
						"Unknown control type (" + a_control.GetType().Name + " in dbGridView:FieldValue",
		...
		public string DataSetValue (int a_row, string a_column)
		{
			...
			return (m_dataSet.Tables[TableName].Rows[a_row][a_column].ToString());
		}		// end of DataSetValue
	...
	// &&&&& put in the proper checks here.
					...
					for (int i=0;  i<m_map.Count;  i++)		// Loop for all columns in the map.
					{
						if (m_map.m_gridColumn[i] != null)	// If grid column is defined...
						{
							...
							m_dataGrid[m_map.m_gridColumn[i], row].Value =
								m_dataSet.Tables[TableName].Rows[row][m_map.m_dbColumn[i]].ToString();
						}
					}
		...
		{
			return (DisplayGridLine (a_row, a_row));
				...
				for (int i=0;  i<m_map.Count;  i++)			// Loop for all columns in the map.
				{
					if (m_map.m_gridColumn[i] != null)		// If grid column is defined...
					{
						...
						m_dataGrid[m_map.m_gridColumn[i], a_gridRow].Value =
							m_dataSet.Tables[TableName].Rows[a_dataSetRow][m_map.m_dbColumn[i]].ToString();
					}
				}
...
// use rowenter event
		{
            if (alreadyHere == false)
            {
                alreadyHere = true;
                GridSelectionHandlerEnabled = false;
                int row = GridCurrentRow();						// Index of current row.
                Debug.Assert(
		...
		// &&&&& add error checking for null grid, no rows selected, etc.
			return (m_dataGrid.SelectedCells[0].RowIndex);
...
//			get	{	return (m_dataGrid.CurrentRow.Index);	}
	...
	//			m_dataGrid.Rows[0].Selected = true;
		...
		/// </summary>
		//-----------------------------------------------------------------------------------------
		private void MapDbColumnsToUiFields ()
		{
// &&&&& this needs to be made recursive to find embedded controls.
		...
		public void MoveDataSetToDisplay (int a_row)
		{
			...
			for (int i=0;  i<m_map.Count;  i++)				// For each column defined in the DB row...
			{
				...
				{
					ControlValue (							// Set UI control value(control,value).
				...
				if (m_map.m_uiControl != null)
				{
					if (m_map.m_uiControl[i] != null)
						m_dataSet.Tables[TableName].Rows[row][m_map.m_dbColumn[i]] = 
							ControlValue (m_map.m_uiControl[i]);
				}
			}
			DisplayGridLine (row);							// Move DataSet row data to grid row.
		...
		public bool RowInsert ()
		{
			...
			m_dataGrid.Rows[gridRowNo].Selected = true;		// Set the new row to be the current row.
			...
			// The data in the UI fields are now in the DataSet row but not displayed on the grid
			// row. The next call displays the new data in the DataSet in the DataGridView row.
			DisplayGridLine (dsRowNo);
			GridSelectionHandlerEnabled = true;
			return (true);
		}		// end of RowInsert
		...
		{
			return (m_map.AddGridColumn (a_dbColumn, a_gridColumn));
