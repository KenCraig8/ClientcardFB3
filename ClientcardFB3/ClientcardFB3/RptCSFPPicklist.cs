...
namespace ClientcardFB3
{
    class RptCSFPPicklist
    {
        ...
        public RptCSFPPicklist(DataGridView dgvIn)
        {
            dgv = dgvIn;
        }
            ...
            string foodBankName, DateTime delieveryDate, string route)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dgv.Rows.Count; i++)
                {
                    ...
                    table.Cell(row, 4).Range.Text = dgv.Rows[i].Cells["clmAptNum"].Value.ToString();
                    table.Cell(row, 5).Range.Text = dgv.Rows[i].Cells["clmPhone"].Value.ToString();
