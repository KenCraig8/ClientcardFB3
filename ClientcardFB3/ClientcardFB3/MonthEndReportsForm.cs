...
//Need To Redo This Code
        ...
        private void fillYearsCombo()
        {
            ...
            string fiscalyear = CCFBGlobal.CurrentFiscalStartDate().Year.ToString();
...
//                        rowIndex = 1;
                ...
                colMinus = 0;
//                lbsInkind = 0;
                if (dtblFoodRecieptsByFunding.Rows.Count > 1)
                {
                        ...
                        if (tb.Tag.ToString() != "")
                        {
                            ...
                            if (dtblFoodRecieptsByFunding.Columns.IndexOf(tagVal) >= 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (tagVal == "LbsFeed" || tagVal == "LbsShared" || tagVal == "LbsWaste")
                                        colMinus += Convert.ToDecimal(dtblFoodRecieptsByFunding.Rows[1][tagVal]);
                                    else
        ...
        {
            return cboReportYear.SelectedItem.ToString() + "-" + strRptMonth() + "_";
        }
        ...
        private void fillServiceCounts(ListView lvNew, ListView lvSvc, DateTime periodStart, DateTime periodEnd)
        {
            ...
            string sqlText = "SELECT DATEPART(yyyy, FirstService) Year, Count(*) NbrClients"
                           + "  FROM Household h "
                           + " WHERE ID in (SELECT DISTINCT HouseholdID FROM TrxLog WHERE TrxDate " + dateRange + ") "
                           + " GROUP BY DATEPART(yyyy, FirstService)"
                           + " ORDER BY DATEPART(yyyy, FirstService)";
            filllvwSvcCounts(Convert.ToDecimal(tbTotalClients.Text), lvNew, sqlText);
            ...
            filllvwSvcCounts(Convert.ToDecimal(tbTotalClients.Text), lvSvc, sqlText);
        }

        private void filllvwSvcCounts(decimal NbrHH, ListView lvw, string sqlQuery)
        {
            lvw.Items.Clear();
            ...
            for (int i = 0; i < dtbl.Rows.Count; i++)
            {
                ...
                lvItm.SubItems.Add((Convert.ToDecimal(dbVal) / NbrHH).ToString("P1"));
                lvw.Items.Add(lvItm);
            }
            lvItm = new ListViewItem("");
