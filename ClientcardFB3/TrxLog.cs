...
/// </summary>
        ...
        {
            get { return drow["Gender"].ToString(); }
            set { drow["Gender"] = value; }
        }
        public Int32 ServiceGroup
        {
            get { return Convert.ToInt32(drow["ServiceGroup"]); }
            set { drow["ServiceGroup"] = value; }
        }
...
/// <returns>Returns if the dataset is populated with valid data</returns>
        ...
        {
            validHHData = true;
            string sql = "SELECT CASE WHEN tl.HHMemId='' THEN h.Name ELSE tl.HHMemId END Name, h.address, h.city, h.state, h.zipcode, h.AptNbr"
                       + ", tl.*, tls.TrxID TrxSigID"
                       + "  FROM TrxLog tl "
                       + "  LEFT JOIN Household h On tl.HouseholdID=h.id "
                       + "  LEFT JOIN TrxLogSig tls ON tl.TrxID = tls.TrxId "
                       + " WHERE tl.TrxDate ='" + trxdate.ToShortDateString() + "'";
            string statuslist = "";
        ...
        {
            SqlCommand cmdDelete = new SqlCommand(" DELETE FROM TrxLog    WHERE TrxId = " + key +"; "
                                                + " DELETE FROM TrxLogSig WHERE TrxId = " + key, conn);
            openConnection();
        ...
        public int update(int hhID, string dateSvc)
        {
            ...
            if (dset.HasChanges() == true)
            {
                try
                {
                    ...
                    {
                        dadAdpt.UpdateCommand = commBuilder.GetUpdateCommand();
                        dadAdpt.InsertCommand = commBuilder.GetInsertCommand();
                    }
...
/// <param name="FieldValue">FieldValue= .Net Data type</param>
    ...
    public class TrxLogSig
    {
        ...
        string textInsert = "INSERT TrxLogSig (TrxId, HouseholdID, SigImage, SigString, Created, CreatedBy)"
                          + "VALUES (@TrxId, @HhID, @Sig, @SigStr, @Created, @CreatedBy)";
        string textLoad = "SELECT * FROM TrxLogSig WHERE TrxId = @TrxId";
        string textUpdate = "UPDATE TrxLogSig Set SigImage = @Sig, SigString = @SigStr, Created = GetDate(), CreatedBy = @CreatedBy "
                          + " WHERE TrxId = @TrxId";
        ...
        public Boolean LoadImage(int newTrxId, int newHhId)
        {
            initSigFields(newTrxId, newHhId);
        ...
        public void initSigFields(int newTrxId, int newHhId)
        {
            haveSig = false;
            TrxId = newTrxId;
            HhID = newHhId;
        ...
        public void Update()
        {
            ...
            sqlUpdateCmd.Parameters.Add(new SqlParameter("@TrxId",sigTrxId));
            sqlUpdateCmd.Parameters.Add(new SqlParameter("@Sig", sigImage));
