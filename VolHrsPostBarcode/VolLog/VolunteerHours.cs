...
namespace ClientcardFB3
{
    public class VolunteerHours
    {
        ...
        public bool checkExistingDateForVol(DateTime trxDate, int volID)
        {
            ...
            {
                openConnection();
                readOnlyCommand = new SqlCommand("Select Count(*) From " + tbName + " Where TrxDate = '" + trxDate + "' And VolID = " + volID.ToString(), conn);
                int nbrRcds = (int)readOnlyCommand.ExecuteScalar();
                closeConnection();
                if (nbrRcds == 0)
                    return false;
            ...
            { 
                CCFBGlobal.appendErrorToErrorReport("Command Text=" + readOnlyCommand.CommandText, ex.GetBaseException().ToString());
                return true;
            }
        }
        ...
        public double TotalHours(string timeIn, string timeOut)
        {
            TimeSpan difference;
            try
            {
                DateTime d = Convert.ToDateTime("1/1/2010 " + timeIn);
                DateTime d2 = Convert.ToDateTime("1/1/2010 " + timeOut);
        ...
        public bool InsertNewRow(int volid, DateTime trxdate, int numvols, double numhours, string timein, string timeout)
        {
            DataRow drownew = dset.Tables[0].NewRow();
            drownew["VolID"] = volid;
            drownew["TrxDate"] = trxdate.ToShortDateString();
            drownew["NumVolunteers"] = numvols;
            drownew["NumVolHours"] = numhours;
            drownew["VolTimeIn"] = timein;
            drownew["VolTimeOut"] = timeout;
            dset.Tables[0].Rows.Add(drownew);
            swhereclause = "WHERE VolId = " + volid + " AND TrxDate = '" + trxdate.ToShortDateString() + "'";
            return insert();
        }
    }
}
